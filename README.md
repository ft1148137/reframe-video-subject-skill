...
Use the skill from Codex, or run the helper script directly from this directory:

```bash
python3 scripts/reframe_subject.py input.mp4 \
  --output output.mp4 \
  --target-description "center dancer in black jacket" \
  --aspect 4:3
```

See `SKILL.md` for the full workflow and ffmpeg fallback commands.
